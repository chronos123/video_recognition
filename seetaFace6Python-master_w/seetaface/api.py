...
LIB_PATH = os.path.join(API_DIR,"lib","win")
os.environ["PATH"] += os.pathsep + LIB_PATH
# dll =CDLL("/home/sxc/pattern/seetaFace6Python-master_w/seetaface/lib/ubuntu/libFaceAPI.so")
dll = CDLL(os.path.join(LIB_PATH, "libFaceAPI.dll"))
...
def get_seetaImageData_by_numpy(image_np:np.array)->np.array:
...
class SeetaFace(object):
    ...
    def _dll_func_def(self):
        ...
        self._SetSingleCalculationThreads.argtypes = (POINTER(SeetaImageData),)
        ...
        self._Predict.argtypes = (POINTER(SeetaImageData) , POINTER(SeetaRect), POINTER(SeetaPointF))

        self._PredictVideo = dll.PredictVideo
        self._PredictVideo.restype = c_int32
        self._PredictVideo.argtypes = (POINTER(SeetaImageData) , POINTER(SeetaRect), POINTER(SeetaPointF))
        ...
        self._mark5.argtypes = (POINTER(SeetaImageData) , POINTER(SeetaRect), POINTER(SeetaPointF))

        self._mark68 = dll.mark68
        self._mark68.restype = c_int32
        self._mark68.argtypes = (POINTER(SeetaImageData) , POINTER(SeetaRect), POINTER(SeetaPointF))

        self._markMask = dll.markMask
        self._markMask.restype = c_int32
        self._markMask.argtypes = (POINTER(SeetaImageData) , POINTER(SeetaRect), POINTER(SeetaPointF),POINTER(c_int32))

        self._CropFace = dll.CropFace
        self._CropFace.restype = SeetaImageData
        self._CropFace.argtypes = (POINTER(SeetaImageData), POINTER(SeetaPointF))

        self._ExtractCroppedFace = dll.ExtractCroppedFace
        self._ExtractCroppedFace.restype = c_int32
        self._ExtractCroppedFace.argtypes = (POINTER(SeetaImageData), POINTER(c_float))

        self._Extract = dll.Extract
        self._Extract.restype = c_int32
        self._Extract.argtypes = (POINTER(SeetaImageData), POINTER(SeetaPointF), POINTER(c_float))
        ...
        self._check.argtypes = (POINTER(SeetaImageData) , POINTER(SeetaRect), POINTER(SeetaPointF))
        ...
        self._PredictGenderWithCrop.argtypes = (POINTER(SeetaImageData), POINTER(SeetaPointF))
        ...
        self._PredictAgeWithCrop.argtypes = (POINTER(SeetaImageData),  POINTER(SeetaPointF))
    ...
    def Detect(self, frame: np.array) -> SeetaFaceInfoArray:
        """
        人脸检测
        :param frame: 原始图像
        :return: 人脸检测信息数组
    ...
    def PredictVideo(self, frame: np.array, face: SeetaRect,points: List[SeetaPointF]) -> int:
        """
        视频rgb活体检测（多帧判断出的结果）
    ...
    def mark5(self, frame: np.array, face: SeetaRect) -> List[SeetaPointF]:
        """
    ...
    def mark68(self, frame: np.array, face: SeetaRect) -> List[SeetaPointF]:
        """
    ...
    def markMask(self, frame: np.array, face: SeetaRect) -> (List[SeetaPointF],int):
        """
    ...
    def CropFace(self, frame: np.array, points: List[SeetaPointF]):
        """
    ...
    def Extract(self, frame: np.array, points: List[SeetaPointF]):
        """
    ...
    def check(self,frame: np.array,face: SeetaRect, points: List[SeetaPointF])->int:
        """
        #检测人脸姿态角度是否合适
        :param frame:
        :param face:
    ...
    def PredictGenderWithCrop(self,frame: np.array,points: List[SeetaPointF]) -> int:
        """
    ...
    def PredictAgeWithCrop(self,frame: np.array, points: List[SeetaPointF]) -> int:
        """
